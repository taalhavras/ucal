...
++  overlapping-in-range
  ...
  (starting-in-range start end m era)
::  +split-single: splits a single moment out from an era.
::
++  split-single
  |=  [=era m=moment]
  ^-  ^era
  ::  even if the era is bounded by # of instances we don't have to change
  ::  anything here.
  =/  excluded=@da  (normalize-da (head (moment-to-range m)))
  era(exdates (~(put in exdates.era) excluded))
::  take an era and a moment generated by it - produce two eras - the first starting
::  at the original and continuing up to (but not including) the target moment
::  and the second that starts at the input moment and continues the original era's
::  rule.
++  split-subsequent
  ...
  =/  res=[moment @ud]
      %-  need
      (successor-in-range d-start d-end orig era)
  ?>  =(-:res derived)
  =/  count=@ud  +:res
  ::  produce type for truncated old era and the new era
  =/  [truncated-era-type=era-type new-era-type=era-type]
      ::  in the %until and %infinite case we truncate the old era
      ::  up to the derived moment and keep the bounds on the
      ::  new era unchanged
      ?:  |(?=([%until *] type.era) ?=([%infinite *] type.era))
