...
++  overlapping-in-range
  ...
  (starting-in-range start end m era)
::  splits a single moment out from an era. returns the modified era
::
++  split-single
  |=  [=era m=moment]
  ^-  ^era
  ...
  ::  anything here.
  =/  excluded=@da  (normalize-da (head (moment-to-start m)))
  era(exdates (~(put in exdates.era) excluded))
::  take an era and a moment generated by it - produce two eras - one starting
::  at the original and continuing up to (but not including) the target moment
::  and a second that starts at the input moment and continues the original era's
::  rule OFFSET from when the moment occurred
++  split-subsequent
  |=  [orig=era m=moment]
  ^-  [era era]
  !!
