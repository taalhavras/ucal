...
const toRelativeTime = (date, referenceTime, unit) =>
  moment(referenceTime).diff(date, unit)
...
const convert = (date, referenceTime) => {
  const percentageOfDay = minsToPct(
    toRelativeTime(date, referenceTime, "minutes")
  )
  const positivePercentage =
    percentageOfDay < 0 ? -percentageOfDay : 100 - percentageOfDay
...
const HoursBar: React.FC<Props> = ({ displayDay, userLocation }) => {
  ...
  const referenceTime = moment(displayDay).startOf("day")
