...
export const addDays = (day: Date, days: number): Date =>
  moment(day).add(days, "days").toDate()

export const getFirstOfWeek = (day: Date): Date =>
  moment(day).subtract(day.getDay(), "days").toDate()

export const getLastOfWeek = (day: Date): Date =>
  moment(day)
    .add(6 - day.getDay(), "days")
    .toDate()

export const spansMonths = (day: Date): boolean =>
  getFirstOfWeek(day).getMonth() !== day.getMonth() ||
  getLastOfWeek(day).getMonth() !== day.getMonth()
...
export const getDefaultStartEndTimes = (): {
  ...
  return {
    ...
    endTime: moment()
      .add(toAdd + 30, "minutes")
      .format("HH:mm"),
  }
}

export const isSameDay = (one: Date, two: Date): boolean =>
  moment(one).isSame(moment(two), "day")

export const sameMonthDay = (one: Date, two: Date): boolean =>
  `${one.getMonth()}${one.getDate()}` === `${two.getMonth()}${two.getDate()}`
